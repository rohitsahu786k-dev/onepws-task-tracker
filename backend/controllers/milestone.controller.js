...
const getAll = asyncHandler(async (req, res) => {
  ...
  const [items, total] = await Promise.all([
    Milestone.find(query).sort({ createdAt: -1, _id: -1 }).skip(skip).limit(limit),
...
const create = asyncHandler(async (req, res) => {
  ...
  const item = await Milestone.create(payload);
  res.status(201).json({ success: true, data: item, milestone: item });
});
...
const getById = asyncHandler(async (req, res) => {
  ...
  res.json({ success: true, data: item, milestone: item });
});
