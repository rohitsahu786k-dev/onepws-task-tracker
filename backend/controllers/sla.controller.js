...
const getAll = asyncHandler(async (req, res) => {
  ...
  const trackers = await SLATracker.find({ workspace: workspaceId })
    .populate('task', 'taskNumber title assignedTo project')
    .sort({ createdAt: -1 });
...
const getById = asyncHandler(async (req, res) => {
  ...
  const tracker = await SLATracker.findOne({ _id: req.params.id, workspace: workspaceId }).populate(
    'task',
    'taskNumber title assignedTo project'
  );
...
const resetT0 = asyncHandler(async (req, res) => {
  ...
  const tracker = await SLATracker.findOne({ _id: req.params.id, workspace: workspaceId });
  if (!tracker) return res.status(404).json({ success: false, message: 'SLA tracker not found' });
