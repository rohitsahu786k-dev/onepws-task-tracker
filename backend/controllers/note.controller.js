...
const getAll = asyncHandler(async (req, res) => {
  ...
  const [items, total] = await Promise.all([
    Note.find(query).sort({ createdAt: -1, _id: -1 }).skip(skip).limit(limit),
...
const create = asyncHandler(async (req, res) => {
  ...
  const item = await Note.create(payload);
  res.status(201).json({ success: true, data: item, note: item });
});
...
const update = asyncHandler(async (req, res) => {
  const item = await Note.findByIdAndUpdate(req.params.id, req.body, {
    new: true,
    runValidators: true,
  });
  if (!item) return res.status(404).json({ success: false, message: 'Note not found' });
