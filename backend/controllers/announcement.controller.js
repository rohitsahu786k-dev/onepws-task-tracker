...
const getAll = asyncHandler(async (req, res) => {
  ...
  const [items, total] = await Promise.all([
    Announcement.find(query).sort({ createdAt: -1, _id: -1 }).skip(skip).limit(limit),
...
const create = asyncHandler(async (req, res) => {
  ...
  res.status(201).json({ success: true, data: item, announcement: item });
});

const getById = asyncHandler(async (req, res) => {
  const item = await Announcement.findById(req.params.id);
  if (!item) return res.status(404).json({ success: false, message: 'Announcement not found' });
  res.json({ success: true, data: item, announcement: item });
});
