...
const getAll = asyncHandler(async (req, res) => {
  ...
  const [items, total] = await Promise.all([
    Timesheet.find(query).sort({ createdAt: -1, _id: -1 }).skip(skip).limit(limit),
...
const create = asyncHandler(async (req, res) => {
  ...
  const item = await Timesheet.create(payload);
...
const update = asyncHandler(async (req, res) => {
  const item = await Timesheet.findByIdAndUpdate(req.params.id, req.body, {
    new: true,
    runValidators: true,
  });
  if (!item) return res.status(404).json({ success: false, message: 'Timesheet not found' });
