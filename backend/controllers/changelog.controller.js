...
const getAll = asyncHandler(async (req, res) => {
  ...
  const [items, total] = await Promise.all([
    Changelog.find(query).sort({ createdAt: -1, _id: -1 }).skip(skip).limit(limit),
...
const create = asyncHandler(async (req, res) => {
  ...
  const item = await Changelog.create(payload);
  res.status(201).json({ success: true, data: item, changelog: item });
});

const getById = asyncHandler(async (req, res) => {
  const item = await Changelog.findById(req.params.id);
  if (!item) return res.status(404).json({ success: false, message: 'Changelog not found' });
  res.json({ success: true, data: item, changelog: item });
});

const update = asyncHandler(async (req, res) => {
  const item = await Changelog.findByIdAndUpdate(req.params.id, req.body, {
    new: true,
    runValidators: true,
  });
  if (!item) return res.status(404).json({ success: false, message: 'Changelog not found' });
